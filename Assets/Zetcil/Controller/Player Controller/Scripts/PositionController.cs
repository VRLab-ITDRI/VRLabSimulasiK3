...
namespace Zetcil
{
    ...
    public class PositionController : MonoBehaviour
    {
        ...
        public enum CVectorStyle { None, StyleFPS, StyleADV, StyleMMO, StyleSSP, StyleRPG, StyleJRPG }
        ...
        bool VectorJump = false;
        bool VectorJumpWait = false;
        ...
        void FixedUpdate()
        {
            if (isEnabled && TargetHealth.CurrentValue > 0)
            {
                ...
                if (InputType == CInputType.Vector)
                {
                    ...
                    xMovementTargetVector = TargetVector.CurrentValue.x; // The horizontal movement from joystick 01
                    zMovementTargetVector = TargetVector.CurrentValue.z; // The vertical movement from joystick 01	

                    moveDirection.x = 0;
                    moveDirection.z = 0;

                    //if (xMovementTargetVector != 0 && zMovementTargetVector != 0)
                    //{
                    if (VectorStyle == CVectorStyle.StyleFPS)
                        {
                            moveDirection = TargetController.transform.TransformDirection(TargetVector.CurrentValue) * MoveSpeed * 100 * Time.deltaTime;

                            if (VectorJump)
                            {
                                VectorJump = false;
                                VectorJumpWait = true;
                                Invoke("JumpWaitCooldown", 0.1f);
                        ...
                        else if (VectorStyle == CVectorStyle.StyleADV)
                        {
                            ...
                            Vector3 TargetVectorInput = new Vector3(xMovementTargetVector, 0, zMovementTargetVector);
                            ...
                            if (zMovementTargetVector > 0 || zMovementTargetVector < 0)
                            {
                        ...
                        else if (VectorStyle == CVectorStyle.StyleMMO)
                        {
                            float tempAngle = Mathf.Atan2(zMovementTargetVector, xMovementTargetVector);
                            xMovementTargetVector *= Mathf.Abs(Mathf.Cos(tempAngle));
                            zMovementTargetVector *= Mathf.Abs(Mathf.Sin(tempAngle));
                        ...
                        else if (VectorStyle == CVectorStyle.StyleRPG)
                        {
                            float tempAngle = Mathf.Atan2(zMovementTargetVector, xMovementTargetVector);
                            xMovementTargetVector *= Mathf.Abs(Mathf.Cos(tempAngle));
                            zMovementTargetVector *= Mathf.Abs(Mathf.Sin(tempAngle));
                        ...
                        else if (VectorStyle == CVectorStyle.StyleSSP)
                        {
                            float tempAngle = Mathf.Atan2(zMovementTargetVector, xMovementTargetVector);
                            xMovementTargetVector *= Mathf.Abs(Mathf.Cos(tempAngle));
                            zMovementTargetVector *= Mathf.Abs(Mathf.Sin(tempAngle));
        ...
        void JumpWaitCooldown()
        {
            VectorJumpWait = false;
        }
    }
}
